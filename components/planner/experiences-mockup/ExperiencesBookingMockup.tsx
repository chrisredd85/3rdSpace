    ...
    <div className="min-h-full bg-cream text-ink">
      <div className="border-b border-tan bg-cream/90 px-4 py-4 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <p className="font-mono text-xs font-semibold uppercase tracking-[0.28em] text-ink-faint sm:text-sm">
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <p className="font-mono text-xs font-semibold uppercase tracking-[0.24em] text-ink-faint">
                  {event.dateLabel} · {event.metro}
                </p>
                <span className={cn('mt-1 h-2.5 w-2.5 rounded-full', dotClass(event.tone))} />
              </div>
              <h2 className="mt-4 truncate font-display text-2xl text-ink">{event.title}</h2>
              <div className="mt-3 flex items-center justify-between gap-3">
                <p className="font-mono text-xs font-semibold uppercase tracking-[0.16em] text-ink-faint">{event.timingLabel}</p>
                <p className={cn('font-mono text-xs font-semibold uppercase tracking-[0.16em]', attentionClass(event.tone))}>
                  {attentionLabel}
                </p>
              </div>
            </button>
...
function EventHeader() {
  return (
    <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
      <div>
        <h1 className="font-display text-5xl leading-[1.05] text-ink sm:text-6xl">{founderDinner.title}</h1>
        <p className="mt-4 max-w-3xl text-lg text-ink-soft sm:text-xl">
          {founderDinner.dateLabel} · {founderDinner.description}
        </p>
      </div>
      <div className="flex flex-wrap gap-2 lg:justify-end">
        {founderDinner.chips.map((chip) => (
          <span key={chip} className="rounded-full border border-tan bg-cream px-4 py-2 font-mono text-xs font-semibold uppercase tracking-[0.12em] text-ink-soft">
            {chip}
          </span>
        ))}
      </div>
  ...
  return (
    ...
    >
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div>
          <p className={cn('font-mono text-sm font-semibold uppercase tracking-[0.24em]', bannerState === 'calm' ? 'text-forest' : 'text-clay-deep')}>
            What needs you
          </p>
          <h2 className="mt-4 max-w-4xl text-2xl font-semibold leading-snug text-ink sm:text-3xl">{banner.eyebrow}</h2>
          <p className="mt-2 max-w-4xl text-base leading-7 text-ink sm:text-lg">{banner.title}</p>
...
function SectionRow({ section, isOpen, onClick }: { section: DrilldownSection; isOpen: boolean; onClick: () => void }) {
  ...
  return (
    ...
    >
      <span className="font-mono text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint">{section.number}</span>
      <span className="min-w-0">
        <span className="flex items-center gap-3">
          <Icon className="h-5 w-5 text-ink-faint sm:hidden" />
          <span className="font-display text-3xl leading-tight text-ink sm:text-4xl">{section.title}</span>
        </span>
        <span className="mt-1 block text-base leading-6 text-ink-soft sm:text-lg">{section.summary}</span>
      </span>
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0">
        ...
        <span className="mt-1 block text-base text-ink-faint">{item.detail}</span>
      </span>
      <span className="font-mono text-base font-semibold text-ink sm:text-lg">
        {item.costCents === null ? '—' : formatCents(item.costCents)} {item.costLabel ? <span className="text-xs uppercase tracking-[0.12em] text-ink-faint">{item.costLabel}</span> : null}
      </span>
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem]">
      <div className="min-w-0">
        <p className="font-display text-2xl text-ink">{venueDetail.title}</p>
      ...
      <div className="rounded-lg border border-clay/25 bg-clay-tint/45 p-4">
        <p className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-clay-deep">Approval required</p>
...
function PhotoDetail() {
  return (
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
      <div>
        <p className="font-display text-2xl text-ink">{photoDetail.title}</p>
...
function FoodDetail() {
  return (
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
      <div>
        <p className="font-display text-2xl text-ink">Souvla catering + house bar</p>
...
function CheckinDetail() {
  return (
    <div>
      <p className="font-display text-2xl text-ink">House staff check-in</p>
...
function DetailMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-tan bg-cream p-4">
      <p className="font-mono text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint">{label}</p>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
      <div className="rounded-lg border border-tan bg-cream p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-ink-faint">Projected profit</p>
            <p className="mt-2 font-display text-5xl text-forest">{formatCents(moneySummary.projectedProfitCents)}</p>
          </div>
      ...
      <aside className="rounded-lg border border-ochre/25 bg-ochre-tint/45 p-5">
        <p className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-ochre">Money watch</p>
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
      ...
      <aside className="rounded-lg border border-tan bg-cream p-5">
        <p className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-ink-faint">Reminders</p>
...
function StatusPill({ tone, children }: { tone: StatusTone; children: string }) {
  return (
    <span className={cn('inline-flex w-fit items-center rounded-full px-3 py-1.5 font-mono text-xs font-semibold uppercase tracking-[0.16em]', pillClass(tone))}>
      {children}
    </span>
  )
}
...
function PlaceholderEventState({ event, onReturn }: { event: MockupEventCard; onReturn: () => void }) {
  return (
    <section className="rounded-lg border border-tan bg-cream p-6 shadow-card sm:p-8">
      ...
      <h1 className="mt-4 font-display text-5xl leading-tight text-ink">{event.title}</h1>
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-ink/30 px-4 py-6 sm:items-center" role="dialog" aria-modal="true" aria-labelledby="approval-modal-title">
      <div className="w-full max-w-md rounded-lg border border-tan bg-cream p-5 shadow-card">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-clay-deep">Approval confirmation</p>
            <h2 id="approval-modal-title" className="mt-3 font-display text-3xl text-ink">Approve hold?</h2>
          </div>
