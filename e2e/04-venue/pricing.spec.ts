...
test.describe('venue pricing workflow', () => {
  test('bar owner can set a $3 per-head kickback rate', async ({ page }, testInfo) => {
    ...
    annotateFailure(testInfo, 'PAYOUT_STATE_ERROR', 'Venue per-head kickback settings must persist before attendee upload')
    ...
    await page.getByRole('button', { name: /revenue share/i }).click()
    await page.locator('input[name="per_head_kickback"]').fill(String(e2ePersonas.venue.perHeadKickback))

    await expect(
      page.getByText(`Per-head kickback: $${e2ePersonas.venue.perHeadKickback * businessScenarios.verifiedAttendance}`)
    ).toBeVisible()
