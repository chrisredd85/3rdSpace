...
export const responseAnalysisAgentOutputSchema = z.object({
  ...
  service_type: z.string().trim().min(1).nullable().default(null),
  quoted_price_cents: centsSchema,
  minimum_spend_cents: centsSchema,
  deposit_required_cents: centsSchema,
  availability_notes: z.string().trim().min(1).nullable().default(null),
...
const RESPONSE_ANALYSIS_SYSTEM_PROMPT = [
  ...
  'For vendor replies, extract service_type, quoted_price_cents, deposit_required_cents, availability_notes, included_services, and notes when present.',
