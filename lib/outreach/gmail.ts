...
import type { CreatorEmailAccount } from '@/lib/types'
...
function timingSafeEqualString(left: string, right: string) {
  ...
  return createHmac('sha256', getOAuthStateSecret()).update(left).digest('hex') ===
    createHmac('sha256', getOAuthStateSecret()).update(right).digest('hex')
}
