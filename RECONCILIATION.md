...
Status: APPROVED for Phase 0 implementation. This document records the repo reconciliation for the proposed revenue share settlement, screenshot extraction, Stripe Invoicing, payout surfaces, venue compliance gate, and post-audit money-flow scope.
...
RESOLVED — see Approved Decisions and Post-Audit Decisions sections below.
...
### Free tier fix — bundled into Phase 0

The 2-event free tier and the plan creation billing gate are implemented as the first two commits of this PR before any rev share work begins. The earlier attempt at this fix did not land on main.

### Money unit cleanup — bundled into Phase 0

Scope expanded beyond the rev share columns. This PR normalizes all money unit inconsistency across the marketplace:

- vendor_transactions amount fields
- venues hourly_rate / daily_rate / price_per_night
- per_head_kickback variants (`per_head_kickback`, `per_head_kickback_amount`, `per_head_kickback_cents`) → canonicalize on `per_head_kickback_cents`
- Admin catalog seeding routes
- All planner economics helpers

Approach: add `*_cents` columns alongside legacy dollar columns. New code reads/writes only `*_cents`. Legacy columns remain populated by a backfill trigger or one-time UPDATE during migration. No silent multiplications anywhere. Use `dollarsToCents()` / `centsToDollars()` at every dollar/cents boundary.

### Vendor payment backend — Phase 3 UI wiring bundled in

The audit confirmed the vendor payment backend (`app/api/payments/create-intent`, `confirm`, `lib/payments/vendor-payments.ts`) is already built. Phase 3 of this PR wires the planner approval cards to those existing routes. Two competing approaches exist (`create-intent` + `confirm` vs direct `/api/payments/vendor`). This PR canonicalizes on the create-intent + confirm path and deprecates the direct route.

### Builder → Venue rental — Phase 2 bundled in

New flow added: builder pays venue rental fee via Stripe Checkout, funds transfer to venue Connect account. Schema, route, webhook, refund, and UI all in scope.

### Cleanup of Open Questions section

The original "Open Questions For Review" section is superseded by the "Approved Decisions" and "Post-Audit Decisions" sections.

## Approval Gate

CLEARED — approval confirmed. Proceed with Phase 0 only and stop at the Phase 0 draft PR checkpoint before Phase 1.
