...
describe('getMockAgentResponse', () => {
  ...
  it('turns recommend model into commercial model comparison metadata', () => {
    const content =
      'Founder mixer for 80 founders in SOMA next Friday evening around $8k. Ticketing model: paid tickets. Food responsibility: cash bar. Vendor needs: no vendors. Amenities: AV and check-in. Venue terms: flexible. Revenue model: recommend model. Agent action: show options only. Audience is founders and operators. Drinks-only.'
    const message: PlanMessage = {
      id: 'message-1',
      plan_id: 'plan-1',
      role: 'user',
    ...
    expect(recommendations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          recommended_commercial_model: expect.stringMatching(/bar revenue share|ticket revenue share|per-head/i),
          commercial_model_options: expect.arrayContaining(['Flat rental', 'Minimum spend', 'Per-head kickback']),
        }),
      ])
    )
  })
