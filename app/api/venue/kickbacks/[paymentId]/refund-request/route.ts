...
export async function POST(
  ...
  try {
    ...
    if (!agreement) return NextResponse.json({ error: 'Kickback agreement not found' }, { status: 404 })
    ...
    await sendBuilderRefundRequestEmail({ paymentId: payment.id }).catch((error) => {
      console.error('[kickback.refund-request] Failed to send builder refund email', error)
    })

    return NextResponse.json({ payment: updated })
  } catch (error) {
    console.error('[kickback.refund-request] Failed', error)
