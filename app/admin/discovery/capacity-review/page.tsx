...
export default async function CapacityReviewPage() {
  ...
  const { data, error } = await admin
    ...
    .select('id,name,address,website,inferred_capacity_standing,inferred_capacity_seated,capacity_inference_confidence,capacity_inference_source_quote,capacity_inference_admin_status,capacity_inference_extracted_at,updated_at')
    .not('capacity_inference_extracted_at', 'is', null)
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="border-b border-border pb-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Approve, reject, or correct inferred discovery venue capacities before they become trusted planning estimates.
          </p>
        </header>
        ...
        <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          {rows.length === 0 ? (
            <p className="text-sm text-muted-foreground">No venue capacity estimates need review.</p>
          ) : (
                    ...
                    <div className="min-w-0">
                      ...
                      {row.capacity_inference_source_quote ? (
                        ...
                        <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">
                          No source quote was available. This estimate came from name, address, and Places type metadata.
                        </p>
                      )}
                    </div>
                    <div className="grid shrink-0 gap-2 text-sm sm:grid-cols-3 lg:min-w-[360px]">
                      <Metric label="Standing" value={formatCount(row.inferred_capacity_standing)} />
                      <Metric label="Seated" value={formatCount(row.inferred_capacity_seated)} />
                  ...
                  <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_auto_auto]">
                    <form action={editCapacity} className="grid gap-2 sm:grid-cols-3">
                      ...
                      <label className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
                        Standing capacity
                        <input name="standing_capacity" type="number" min="0" step="1" defaultValue={row.inferred_capacity_standing ?? ''} className="mt-1 w-full rounded-md border border-border bg-card px-3 py-2 text-sm text-foreground" />
                      </label>
                      <label className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
                        Seated capacity
                        <input name="seated_capacity" type="number" min="0" step="1" defaultValue={row.inferred_capacity_seated ?? ''} className="mt-1 w-full rounded-md border border-border bg-card px-3 py-2 text-sm text-foreground" />
                      </label>
...
async function editCapacity(formData: FormData) {
  ...
  await (createServiceRoleClient() as any)
    .from('discovery_venues')
    .update({
      inferred_capacity_standing: readOptionalCount(formData.get('standing_capacity')),
      inferred_capacity_seated: readOptionalCount(formData.get('seated_capacity')),
...
function readOptionalCount(value: FormDataEntryValue | null): number | null {
